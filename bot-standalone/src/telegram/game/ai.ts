...
import { isInBounds } from "./engine.js";

const SEARCH_RADIUS = 2;

interface Candidate { row: number; col: number; }
...
function scoreCell(engine: GameEngine, row: number, col: number, player: Cell): number {
  let total = 0;
  for (const [dr, dc] of DIRECTIONS) {
    ...
    if (count >= engine.winLength) total += 1_000_000;
    else if (count === engine.winLength - 1 && openEnds > 0) total += openEnds === 2 ? 50_000 : 8_000;
    else if (count === engine.winLength - 2 && openEnds === 2) total += 1_500;
    else total += count * count * (openEnds + 1) * 10;
  }
  return total;
}

export function pickBotMove(engine: GameEngine, botPlayer: Cell, humanPlayer: Cell): Candidate {
  const candidates = collectCandidates(engine);
  let best: Candidate = candidates[0]!;
  let bestScore = -Infinity;
  for (const { row, col } of candidates) {
    const score = scoreCell(engine, row, col, botPlayer) * 1.05 + scoreCell(engine, row, col, humanPlayer);
