...
export async function upsertGroup(chatId: number, title: string): Promise<TelegramGroup> {
...
export async function listGroups(): Promise<TelegramGroup[]> {
  return db.select().from(telegramGroupsTable).orderBy(desc(telegramGroupsTable.updatedAt));
}
...
export async function trackMember(
  ...
  if (existing) {
    await db
      ...
      .set({ firstName, lastName, username, lastSeenAt: new Date() })
      .where(
        and(
          eq(telegramMembersTable.chatId, chatIdStr),
          eq(telegramMembersTable.userId, userIdStr),
  ...
  await db
    .insert(telegramMembersTable)
    .values({ chatId: chatIdStr, userId: userIdStr, firstName, lastName, username });
}
