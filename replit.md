...
- API: Express 5
- Telegram bot: `node-telegram-bot-api` (polling mode), runs in-process alongside the API server (`artifacts/api-server/src/telegram/`)
...
- `lib/db/src/schema/telegramGroups.ts`, `telegramMembers.ts` — group whitelist + best-effort member roster
...
- Pin `node-telegram-bot-api` to `0.66.0` (with `@types/node-telegram-bot-api@0.64.15`). The `1.x` line is a full rewrite with a different API shape (named type exports instead of a namespace, no classic `polling: true` constructor surface) and breaks this codebase.
