...
export async function cmdListGroups(
  bot: TelegramBot,
  chatId: number,
  userId: number,
): Promise<void> {
  ...
  if (groups.length === 0) {
    await bot.sendMessage(chatId, "Bot chưa được thêm vào nhóm nào.");
    return;
  }
...
export async function cmdSetAllowed(
  ...
  if (!updated) {
    await bot.sendMessage(
      chatId,
      `Không tìm thấy nhóm với ID ${targetChatId}. Bot phải được thêm vào nhóm trước.`,
    );
    return;
  }
  ...
  await bot.sendMessage(
    chatId,
    allowed
      ? `Đã kích hoạt bot cho nhóm ${targetChatId}.`
      : `Đã tắt bot ở nhóm ${targetChatId}.`,
  );
  ...
  if (allowed) {
    try {
      await bot.sendMessage(
        Number(targetChatId),
        "\u2705 Nhóm này đã được admin kích hoạt! Gõ /join hoặc /joinbot để bắt đầu chơi.",
      );
    } catch (err) {
      logger.warn({ err }, "Could not notify group about activation");
    }
  }
}
...
export async function cmdMembers(
  ...
  if (members.length === 0) {
    await bot.sendMessage(chatId, "Chưa ghi nhận thành viên nào trong nhóm này.");
    return;
  }
...
export async function cmdExportMembers(
  ...
  const content = [
    `Danh sách thành viên nhóm ${targetChatId}`,
    "Tên\tUsername\tUserID",
    ...members.map(
      (m) => `${memberDisplayName(m)}\t${m.username ? `@${m.username}` : "-"}\t${m.userId}`,
    ),
  ].join("\n");

  const tmpFile = path.join(
    os.tmpdir(),
    `telegram-members-${targetChatId}-${Date.now()}.txt`,
  );

  try {
    await fs.writeFile(tmpFile, content, "utf-8");
    await bot.sendDocument(chatId, tmpFile, {
      caption: `Danh sách thành viên nhóm ${targetChatId} (${members.length} người)`,
    });
  } finally {
    // Always clean up the temp export file after sending it.
...
export async function cmdKick(
  ...
  try {
    ...
    await bot.unbanChatMember(targetChatId, Number(targetUserId), {
      only_if_banned: true,
    });
    await removeMember(targetChatId, targetUserId);
    await bot.sendMessage(
      chatId,
      `Đã xoá thành viên ${targetUserId} khỏi nhóm ${targetChatId}.`,
    );
  } catch (err) {
    logger.error({ err }, "Failed to kick member");
    await bot.sendMessage(
      chatId,
      "Không thể xoá thành viên. Kiểm tra bot đã là quản trị viên của nhóm đó chưa.",
    );
  }
}
...
export async function cmdMute(
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `Đã cấm chat thành viên ${targetUserId} trong nhóm ${targetChatId}${
        minutes ? ` trong ${minutes} phút` : " (vĩnh viễn)"
      }.`,
    );
  } catch (err) {
    ...
    await bot.sendMessage(
      chatId,
      "Không thể cấm chat thành viên. Kiểm tra bot đã là quản trị viên của nhóm đó chưa.",
    );
  }
}
...
export async function cmdUnmute(
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `Đã bỏ cấm chat cho thành viên ${targetUserId} trong nhóm ${targetChatId}.`,
    );
  } catch (err) {
...
export async function cmdBroadcast(
  bot: TelegramBot,
  chatId: number,
  userId: number,
  ...
  const targets = targetChatId
    ? [targetChatId]
    : (await listGroups()).filter((g) => g.allowed).map((g) => g.chatId);

  let sent = 0;
  for (const target of targets) {
    try {
      await bot.sendMessage(Number(target), `\u{1F4E2} ${text}`);
      sent++;
    } catch (err) {
      logger.warn({ err, target }, "Failed to send broadcast to group");
    }
  }
